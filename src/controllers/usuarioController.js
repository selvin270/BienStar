...
export const crearUsuario = [
  ...
  async (req, res) => {
    try {
      const {
        nombre,
        apellido = null, // Si no se envía, será NULL en la BD
        correo,
        contrasenia,
        edad = null,
        genero = null,
      ...
      if (!nombre || !correo || !contrasenia) {
        return res.status(400).json({
          tipo: "error",
          msj: "Ingrese los campos obligatorios",
        });
      }
      ...
      // Insertar usuario con valores opcionales manejados correctamente
      await connection.query(
        "INSERT INTO usuario (nombre, apellido, correo, contrasenia, edad, genero, estatura, peso, avatar) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
        [
          nombre,
          apellido || null, // Si no viene, insertar NULL
          correo,
          contrasenia,
          edad || null,
          genero || null,
  ...
  async (req, res) => {
    ...
    try {
      ...
      const [userRows] = await connection.query(
        "SELECT nombre, apellido, correo, edad, genero, estatura, peso, avatar FROM usuario WHERE id_usuario = ?",
        [id_usuario]
      );
      ...
      // Convertir valores vacíos en NULL o conservar los valores actuales
      const nombreFinal =
        nombre && nombre.trim() !== "" ? nombre : usuarioActual.nombre;
      const apellidoFinal =
        apellido && apellido.trim() !== "" ? apellido : usuarioActual.apellido;
      const correoFinal =
        correo && correo.trim() !== "" ? correo : usuarioActual.correo;
      const edadFinal =
        edad && !isNaN(edad) ? parseInt(edad, 10) : usuarioActual.edad;
      const generoFinal =
        genero && genero.trim() !== "" ? genero : usuarioActual.genero;
      const estaturaFinal =
        estatura && !isNaN(estatura)
          ? parseInt(estatura, 10)
          : usuarioActual.estatura;
      const pesoFinal =
        peso && !isNaN(peso) ? parseInt(peso, 10) : usuarioActual.peso;
      const avatarFinal = avatar || usuarioActual.avatar; // Conservar el avatar actual si no se sube uno nuevo
...
export const recuperarContrasenia = async (req, res) => {
  ...
  try {
    ...
    const resetLink = `http://192.168.10.216:4000/restablecer-contrasenia/${token}`;
